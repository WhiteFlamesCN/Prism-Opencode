...
describe("tool.edit", () => {
  ...
  describe("edge cases", () => {
    ...
    test("preserves block indentation when oldString is unindented", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "code.txt")
      const original = [
        "class Query:",
        "    def add_q(self, q):",
        ...
        fn: async () => {
          ...
          await Effect.runPromise(
            edit.execute(
              {
                filePath: filepath,
                // Agent provides oldString with 0-space leading indent
                oldString: [
                  "child_clause = self._add_q(",
                  "    child, used_aliases,",
                  "    current_negated, allow_joins)",
                ...
                newString: [
                  "child_clause = self._add_q(",
                  "    child, used_aliases,",
                  "    current_negated, allow_joins, simple_col=simple_col)",
                ].join("\n"),
              },
              ctx,
            ),
          ...
          // The 16-space indent of the original block must be preserved
          expect(after).toContain("                child_clause = self._add_q(")
          expect(after).toContain("                    child, used_aliases,")
          expect(after).toContain("                    current_negated, allow_joins, simple_col=simple_col)")
          // Make sure no flush-left version snuck in
          expect(after.split("\n").every((l) => !l.startsWith("child_clause"))).toBe(true)
        },
      })
    })
  })
