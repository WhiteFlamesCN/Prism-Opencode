...
function minLeadingIndent(text: string): number {
  const lines = text.split("\n")
  let min = Infinity
...
function reindent(text: string, delta: number): string {
  ...
  if (delta > 0) {
    ...
    return lines.map((l) => (l.trim().length === 0 ? l : pad + l)).join("\n")
  }
  const strip = -delta
  return lines
    .map((l) => {
      ...
      const m = l.match(/^[ \t]*/)
      const w = m ? m[0].length : 0
      return l.slice(Math.min(strip, w))
    })
    .join("\n")
}
...
export function replace(content: string, oldString: string, newString: string, replaceAll = false): string {
  ...
  const findIndent = minLeadingIndent(oldString)
  ...
  for (const replacer of [
    ...
    for (const search of replacer(content, oldString)) {
      ...
      if (index === -1) continue
      notFound = false
      const matchIndent = minLeadingIndent(search)
      const adjustedNew = reindent(newString, matchIndent - findIndent)
      if (replaceAll) {
        return content.replaceAll(search, adjustedNew)
      }
      const lastIndex = content.lastIndexOf(search)
      if (index !== lastIndex) continue
      return content.substring(0, index) + adjustedNew + content.substring(index + search.length)
    }
  }
