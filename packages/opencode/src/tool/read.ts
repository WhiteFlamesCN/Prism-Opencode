...
export const ReadTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ReadTool.execute")(function* (params: z.infer<typeof parameters>, ctx: Tool.Context) {
      ...
      // --- File History: Record read for future summary generation ---
      if (history.isEnabled()) {
        yield* history.recordRead(filepath, ctx.agent, ctx.sessionID)
      }
    ...
    const description = process.env.OPENCODE_FILE_HISTORY === "1"
      ? DESCRIPTION + "\n- BEFORE reading a file you might not need to edit, consider using `read_summary` instead — it returns a concise overview of the file. Use `read` for files you'll actually modify."
      : DESCRIPTION
